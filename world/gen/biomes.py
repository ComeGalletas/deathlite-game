"""Which biome each terrace of an island wears, decided at generation.
...
# What a sheet *is* -- its biome, its surf, its scatter mix -- is a lookup
# shared with the tile painter and the decor, so it lives in `world.rules`.
# Re-exported here for the callers that grew up with these names.
from world.rules.biome import biome_of, has_shoreline, scatter_mix  # noqa: F401
...
def assign_palettes(rooms, seed, settings=None) -> None:
    ...
    for room in rooms:
        if not room.topography:
            continue
        spec = settings_or_config(settings).topography(room)
