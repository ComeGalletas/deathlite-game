...
so the repo root stays clean — the same arrangement as [`../web/`](../web/) for
the browser build. Design notes and the decision log are in
...
| `build.ps1` | Build, report the size, optionally ZIP it. Pins the Python to `.venv` and keeps PyInstaller out of `build/`. |
...
Add `-Zip` to also produce `dist/DeathliteGame-0.5.zip` (the version is read out
...
Output is `dist/DeathliteGame/` — a folder containing `DeathliteGame.exe` and an
...
exe with a console attached, into `dist_console/` so it can never be mistaken
for the shipping one:

```powershell
powershell -ExecutionPolicy Bypass -File desktop\build.ps1 -Console
```
...
**PyInstaller's work directory is redirected.** Its default is `build/`, which
the pygbag build already owns (`build/web`, `build/web-cache`). `build.ps1`
sends it to `build/pyinstaller` and deletes it afterwards unless you pass
`-KeepWork`.
...
ignored. This matters more than it sounds: `assets/` also holds
`unused/` (40 MB) and `Super Pixel Effects Gigapack/` (254 MB, 61,783 files),
neither of which the game references. A deny-list would let the next pack
dropped in there silently join the build. **If you add art in a new top-level
folder under `assets/`, add it to `ASSET_DIRS` or it will not ship.**

The shipped payload is ~323 files and ~7 MB of art and data.
