...
| File | Responsibility |
|------|----------------|
| `systems/audio.py` | `AudioManager`: synthesises 8 cues (shoot/hit/enemy death/xp/level-up/hurt/boss spawn/boss death) from sine/square/noise primitives into raw int16 buffers — **mostly** no audio files. Re-inits the mixer to 44100 Hz stereo (raised from 22050 mono for the streamed music, 2026-09-16; the cues are resampled up in `make_sound`). Degrades to a silent no-op if the mixer can't open (dummy driver / no device). Since 2026-09-16 also loads the recorded cues in `config.SOUND_EFFECTS` from `assets/sound_effects/` into the same library, where a recorded name replaces a synthesised one (`boss_spawn` is the growl now). `play(name, gain=)` sets the level on the channel so one recording can serve two moments. Owns `_Footsteps` (a step per 64 px travelled, takes alternating) and the room-wake growl. Subscribes to the event bus; per-cue min-gap throttle for spammy cues. |
| `systems/music.py` | `MusicPlayer`: streams the two background tracks in `assets/music/` through `pygame.mixer.music` (never loaded as `Sound` -- they decode to ~26 MB each). `play()` is idempotent, so sibling screens sharing a track never restart it; fades are a gain ramp walked by `update(dt)` rather than SDL's blocking `fadeout()`. Silent backend or missing file -> silent, never raises. Which track plays is declared per state (`State.music`) and applied by `StateMachine`. |
