...
- [ ] W9 — `.nojekyll` + `.github/workflows/deploy-web.yml` (sketch above); enable Pages (GitHub Actions source) *(DOC-003: still open and not started; it needs a GitHub Actions workflow, so whether to build it is the owner's call given the no-CI rule)*
