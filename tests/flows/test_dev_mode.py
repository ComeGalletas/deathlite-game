...
class DevMenuTests(unittest.TestCase):
    ...
    def test_no_damage_keeps_the_hero_attacking_but_deals_zero(self):
        ...
        _key(game, pygame.K_BACKQUOTE)
        # Not a fixed offset, for the reason `tests/nearby.py` gives: a
        # Shellback 30 px east can be over a drop, and one that walks off is
        # never hit, so the sanity check at the end sees no damage at all.
        spot = spots_near(playing, want=1, radius=24.0)
        self.assertTrue(spot, "nowhere beside the hero to stand a Shellback")
        playing._spawn_enemy("turtle", at=spot[0])
        e = playing.enemies[-1]
        hp0 = e.hp
        attacked = False
